...
ensure_rootless_storage_conf() {
  # Rootless storage configuration: prefer overlay+fuse if available; fall back to vfs otherwise.
  local conf="$HOME/.config/containers/storage.conf"
  local have_fuse=0
  if command -v fuse-overlayfs >/dev/null 2>&1; then have_fuse=1; fi

  mkdir -p "$(dirname "$conf")"

  if [ -f "$conf" ]; then
    # If fuse is missing but config still uses overlay+mount_program, switch to vfs automatically.
    if [ $have_fuse -eq 0 ] && grep -q 'driver *= *"overlay"' "$conf"; then
      warn "fuse-overlayfs not found; switching rootless storage driver to 'vfs' in $conf"
    else
      log "storage.conf already exists at $conf"
      return
    fi
  else
    log "Writing rootless storage.conf to $conf"
  fi
...
EOF
    ...
    cat > "$conf" <<'EOF'
# Auto-generated by check_podman.sh (vfs fallback)
[storage]
driver = "vfs"
runroot = "$HOME/.local/share/containers/runroot"
graphroot = "$HOME/.local/share/containers/storage"
...
additionalimagestores = []
EOF
    warn "Rootless storage driver set to 'vfs'. If you previously used 'overlay',"
    warn "you may see a driver mismatch warning. To reset Podman store for rootless, run:"
    warn "  podman system reset --force"
  fi
}
